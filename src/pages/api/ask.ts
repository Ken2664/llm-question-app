                ...
                const completion = await Promise.race([
                    deepSeek.chat.completions.create({
                        messages: [
                            ...
                            { role: "user", content: question } // { role: "user", content: `{propmpt}{role description}${courseName}に対する深い知識を持った教授{/role description}{task}与えられたquestionの内容に対して${courseName}の教授としての立場から、questionに対してstep-by-stepで分析して答えてください。{/task}{conditions}・Think in English and answer in Japanese.・質問の内容に対して完全に回答してください。・質問が答えを求めている場合は、正しく答えを教えてください。・答えるときは、step-by-stepの思考手順を簡潔に解説しながら説明してください。・数学や理系科目の説明の際は可能な限り数式を用いて説明するようにしてください。{/conditions}{response conditions}・日本語で回答を出力してください・数式を表示する時はMarkdownとTeXを組み合わせて出力してください。・読みやすく段落分けされた回答をMarkdownを用いて出力してください。{/response conditions}{question}${question}{/question}{/prompt}` }
                        ],
